...
object XXTEA {
    ...
    fun encrypt(data: String, key: ByteArray): ByteArray? =
        runCatching { encrypt(data.toByteArray(Charsets.UTF_8), key) }.getOrNull()

    fun encrypt(data: ByteArray, key: String): ByteArray? =
        runCatching { encrypt(data, key.toByteArray(Charsets.UTF_8)) }.getOrNull()

    fun encrypt(data: String, key: String): ByteArray? =
        runCatching { encrypt(data.toByteArray(Charsets.UTF_8), key.toByteArray(Charsets.UTF_8)) }.getOrNull()
